...
from modules.cadastros.clientes.service import (
    excluir_aniversariante,
    excluir_cliente,
    listar_aniversariantes,
    listar_clientes,
    obter_cliente,
    salvar_aniversariante,
    salvar_cliente,
)
...
def carregar_graus_relacionamento():

    caminho = (
        Path(__file__).resolve().parents[3]
        / "config"
        / "graus_relacionamento.json"
    )
    ...
    graus = dados.get("graus", [])

    return [
        grau
        for grau in graus
        if grau.get("codigo") and grau.get("nome")
    ]
...
def clientes_view(page):
    ...
    lista_aniversariantes = ft.Column(
        spacing=4,
    )
    ...
    graus_relacionamento = carregar_graus_relacionamento()
    graus_por_codigo = {
        grau["codigo"]: grau
        for grau in graus_relacionamento
    }
    ...
    aniversariante_editando = {
        "id": None,
    }

    dd_aniv_relacao = ft.Dropdown(
    ...
    txt_aniv_nome = ft.TextField(
        label="Nome do aniversariante",
        width=360,
    )

    txt_aniv_data_nascimento = ft.TextField(
        label="Data de nascimento",
        hint_text="DD/MM/AAAA",
        width=160,
    )

    chk_aniv_ativo = ft.Checkbox(
        label="Ativo",
        value=True,
    )
    ...
    def validar_cpf_cliente(e=None):
    ...
    txt_cpf.on_blur = validar_cpf_cliente
    ...
    def limpar(e=None):
        ...
        txt_cpf.error = None
        limpar_aniversariante()
        carregar_aniversariantes(False)

        page.update()

    def limpar_aniversariante(e=None):

        aniversariante_editando["id"] = None
        dd_aniv_relacao.value = None
        txt_aniv_nome.value = ""
        txt_aniv_data_nascimento.value = ""
        chk_aniv_ativo.value = True

        if e:
            page.update()

    def carregar_aniversariantes(atualizar=True):

        lista_aniversariantes.controls.clear()

        cliente_id = cliente_editando["id"]

        if not cliente_id:
            lista_aniversariantes.controls.append(
                ft.Text(
                    "Salve o cliente para cadastrar aniversariantes.",
                    size=12,
        ...
        for item in aniversariantes:
            item_id = item[0]
            relacao = item[1] or ""
            grau = graus_por_codigo.get(relacao, {})
            nome = item[2] or ""
            data_nascimento = (
                    ...
                    content=ft.Row(
                        ...
                        controls=[
                            ...
                            ft.IconButton(
                                ...
                                height=32,
                                tooltip="Editar",
                                on_click=lambda e, i=item: editar_aniversariante(i),
                            ),
                            ft.IconButton(
                                ...
                                height=32,
                                tooltip="Excluir",
                                on_click=lambda e, id=item_id: remover_aniversariante(id),
                            ),
                        ],
                    ),
                )
    ...
    def editar_aniversariante(item):

        aniversariante_editando["id"] = item[0]
        dd_aniv_relacao.value = item[1] or None
        txt_aniv_nome.value = item[2] or ""
        txt_aniv_data_nascimento.value = (
            item[3].strftime("%d/%m/%Y")
            if item[3]
            else ""
        )
        chk_aniv_ativo.value = bool(item[4])
        page.update()

    def salvar_aniversariante_cliente(e=None):

        try:
            salvar_aniversariante({
                "id": aniversariante_editando["id"],
                "cliente_id": cliente_editando["id"],
                "relacao": dd_aniv_relacao.value,
                "nome": txt_aniv_nome.value,
                "data_nascimento": txt_aniv_data_nascimento.value,
                "ativo": chk_aniv_ativo.value,
            })

            limpar_aniversariante()
            carregar_aniversariantes(False)
            mostrar_mensagem(
                "Aniversariante salvo"
            )
    ...
    def remover_aniversariante(aniversariante_id):
    ...
    def salvar(e=None):
        ...
        try:
            ...
            cliente_editando["id"] = cliente_id
    ...
    return ft.Column(
        ...
        controls=[
            ...
            ft.ResponsiveRow(
                ...
                run_spacing=4,
                controls=[
                    ft.Container(dd_aniv_relacao, col={"md": 3}),
                    ft.Container(txt_aniv_nome, col={"md": 5}),
                    ft.Container(txt_aniv_data_nascimento, col={"md": 2}),
                    ft.Container(chk_aniv_ativo, col={"md": 2}),
                ],
            ),
            ft.Row(
                spacing=8,
                controls=[
                    ft.ElevatedButton(
                        ...
                        icon=ft.Icons.SAVE,
                        on_click=salvar_aniversariante_cliente,
                    ),
                    ft.OutlinedButton(
                        "Limpar aniversariante",
                        icon=ft.Icons.CLEAR,
                        on_click=limpar_aniversariante,
                    ),
                ],
            ),
