...
def endereco_atual(cursor, empresa_id):
...
def atualizar_geolocalizacao_se_necessario(cursor, empresa_id, dados):

    atual = endereco_atual(cursor, empresa_id)
...
def normalizar_dados(dados):
    ...
    for indice in (1, 2, 3):
        ...
        normalizado[f"{prefixo_db}CPF"] = normalizar_cpf(
            dados.get(f"{prefixo_form}_cpf"),
            campo=f"CPF do responsavel {indice}",
        )
...
def existe_documento(documento, empresa_id=None):
    ...
    if empresa_id:
        ...
        cursor.execute(f"""
            SELECT COUNT(*)
            FROM Empresa
            WHERE {sql_documento_normalizado("Documento")} = ?
            AND EmpresaID <> ?
        """, (
            documento,
            empresa_id,
