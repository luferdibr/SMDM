...
def atores_view(page):
    ...
    def aplicar_endereco(dados):
        ...
        txt_cidade.value = dados.get("municipio") or ""
        txt_estado.value = dados.get("uf") or "RS"
    ...
    txt_cpf.on_blur = validar_cpf_ator
    ...
    def editar(ator_id):
        ...
        geo_endereco["latitude"] = dados[12]
        geo_endereco["longitude"] = dados[13]
        geo_endereco["fonte_geo"] = dados[14]
        geo_endereco["data_geo"] = dados[15]
        atualizar_status_geo()
        dd_nacionalidade.value = dados[16] or ""
        txt_telefone.value = dados[17] or ""
        txt_celular.value = dados[18] or ""
        txt_data_nascimento.value = (
            dados[19].strftime("%d/%m/%Y")
            if dados[19]
            else ""
        )
        txt_cpf.value = dados[20] or ""
        txt_rg.value = dados[21] or ""
        txt_email.value = dados[22] or ""
        dd_ensino.value = dados[23] or ""
        dd_sexo.value = dados[24] or ""
